...
KeyTable = function ( oInit )
{
	...
	/*
	 * Variable: _oaoEvents
	 * Purpose:  Event config.cache object, one array for each supported event for speed of searching
	 * Scope:    KeyTable - private
	 */
	var _oaoEvents = {
		"action": [],
	...
	function _fnEventAddTemplate( sKey )
	{
		/*
		 * Function: -
		 * Purpose:  API function for adding event to config.cache
		 * Returns:  -
	...
	function _fnEventRemoveTemplate( sKey )
	{
		/*
		 * Function: -
		 * Purpose:  API function for removing event from config.cache
	 ...
	 * Purpose:  Add an event to the internal config.cache
	 * Returns:  -
	 ...
	 * Purpose:  Remove an event from the event config.cache
	 ...
	 * Purpose:  Look thought the events config.cache and fire off the event of interest
