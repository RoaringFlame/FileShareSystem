...
DayGrid.mixin({
	...
	computeRowLevelLimit: function(row) {
		...
		var rowHeight = rowEl.height(); // TODO: config.cache somehow?
...
function EventManager(options) { // assumed to be a calendar
				...
				if (eventInputs) {
					for (i = 0; i < eventInputs.length; i++) {
						...
						if (abstractEvent) { // not false (an invalid event)
							cache.push.apply(
								cache,
								expandEvent(abstractEvent) // add individual expanded events to the config.cache
							);
						}
					}
				}
	...
	function removeEvents(filter) {
		...
		// Purge event(s) from our local config.cache
